...
exports.WebSocket = exports.Route = exports.Response = exports.Request = exports.RawHeaders = exports.NetworkRouter = void 0;
...
class NetworkRouter {
  ...
  async routeFromHAR(har, options = {}) {
    const harRouter = await _harRouter.HarRouter.create(this._owner._connection.localUtils(), har, options.notFound || 'abort');
  ...
  async handleRoute(route) {
    ...
    for (const routeHandler of routeHandlers) {
      ...
      if (!this._routes.length) this._owner._wrapApiCall(() => this._updateInterception(), true).catch(() => {});
      if (handled) return true;
    }
    return false;
  }
  async _updateInterception() {
    const patterns = [];
    let all = false;
    ...
    await this._owner._channel.setNetworkInterceptionPatterns(all ? {
      patterns: [{
        glob: '**/*'
      }]
    } : {
